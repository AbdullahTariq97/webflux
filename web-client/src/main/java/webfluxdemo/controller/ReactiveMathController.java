...
import webfluxdemo.exception.InvalidValidationException;
...
public class ReactiveMathController {
    ...
    public Mono<Response> findSquareWithValidation(@PathVariable("input") int input){
        if(input < 10 || input > 20){
            throw new InvalidValidationException(((Integer) input).toString());
        }
        return reactiveMathService.getSquare(input);
    }
}
